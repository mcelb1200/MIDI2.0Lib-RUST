...
fn main() -> io::Result<()> {
    ...
    for ump in parser {
        ...
        match wc {
            ...
            4 => writeln!(writer, "MT: {:?}, Grp: {:2}, Len: 4 words | Data: {:08X} {:08X} {:08X} {:08X} ", mt, grp, ump.data[0], ump.data[1], ump.data[2], ump.data[3])?,
            _ => {
                // Safe generic fallback to prevent panics on unexpected lengths,
                // keeping the data bounded to the `ump.data` size
                write!(writer, "MT: {:?}, Grp: {:2}, Len: {} words | Data: ", mt, grp, wc)?;
                for i in 0..wc.min(4) {
