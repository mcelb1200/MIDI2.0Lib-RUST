...
pub fn scale_down(value: u32, src_bits: u8, dst_bits: u8) -> u32 {
    ...
    // ⚡ Bolt Optimization: Replaced explicit boundary branching `if scale_bits >= 32`
    // with a branchless `.unwrap_or(0)` pattern on `checked_shr`. This avoids
    // source-level bounds checks, allowing the compiler to emit faster native
    // branchless shifting logic.
    value.checked_shr(u32::from(scale_bits)).unwrap_or(0)
}
